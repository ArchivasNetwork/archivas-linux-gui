# How to Get .deb Files for the Release

## Current Situation

The Release workflow failed when v1.0.0 was first created (before we fixed the build issues). Now that the fixes are in place, we need to re-run the workflow.
...
This will trigger the Release workflow again with all our fixes, and it should successfully create the packages.

### Option 2: Manual Workflow Trigger (New!)
...
**Just edit and re-save the release** - that's the quickest way to trigger the workflow again!
